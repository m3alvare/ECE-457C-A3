...
from stable_baselines3 import MODELX
...
def train(hyperparams, run_id, base_log_dir, base_model_dir):
    ...
    while True:
        ...
        model.save(f"{model_dir}/MODELX_{TIMESTEPS*episodes}")

def test(model_path):
    print("tester func called")
    env = gym.make("Humanoid-v4", render_mode="human")
    print(model_path)
    model = MODELX.load(model_path, env=env)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train or test MODELX on Humanoid-v4")
    ...
    if args.mode == "train":
        ...
        base_log_dir = "logs/humanoid/MODELX"
